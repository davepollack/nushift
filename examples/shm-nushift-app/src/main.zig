...
pub fn main() usize {
    return main_impl() catch |err| os_nushift.errorCodeFromSyscallError(err);
}

fn main_impl() os_nushift.SyscallError!usize {
    const shm_cap_id = try os_nushift.syscall(.shm_new, .{ .shm_type = os_nushift.ShmType.four_kib, .length = 1 });
    defer _ = os_nushift.syscall_ignore_errors(.shm_destroy, .{ .shm_cap_id = shm_cap_id });

    _ = try os_nushift.syscall(.shm_acquire, .{ .shm_cap_id = shm_cap_id, .address = ACQUIRE_ADDRESS });
    defer _ = os_nushift.syscall_ignore_errors(.shm_release, .{ .shm_cap_id = shm_cap_id });
